...
def mean_assembler(h, f_bar):
    ...
    u = TrialFunction(V)
    v = TestFunction(V)
    a = inner(p * grad(u), grad(v)) * ufl.dx
...
def BigPhiMat(J, grid):
    ...
    ----------
    J : int
        controls the FE mesh size (h=1/J)
    grid : array
...
def sample_gp(n_sim, m, k, grid, par=False, trans=True, tol=1e-9):
    ...
    -------
    array
        array containing `n_sim` samples from a GP with mean m and cov k evaluated on the grid `grid`. Note: the shape of the resulting array is `len(grid) x n_sim`
    """
    # get length of grid
    d = len(grid)
...
def gen_sensor(
    ...
    Y : array
        vector of sensor locations
    u_quad : function
        function to accurately compute the solution u given a realisation of the forcing f
    grid : array
        grid where forcing f is sampled on
    par : bool, optional
...
def m_post_fem_assembler(J, f_bar, k_f, ϵ, Y, v_dat, par=False, trans=True):
    ...
    # (c(x,y)) for y in Y
    B_fem_s, c_fem = fem_cov_assembler_post(
        J, k_f, Y.flatten(), parallel=par, translation_inv=trans
    )
